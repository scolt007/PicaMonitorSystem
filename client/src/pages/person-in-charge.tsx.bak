...
const PersonInCharge: React.FC = () => {
  ...
  // Special effect to handle departments loading
  useEffect(() => {
    if (departments && departments.length > 0) {
      console.log("Departments loaded:", departments);
      // This forces a re-render of the select component
      if (isAddDialogOpen || isEditDialogOpen) {
        form.setValue("departmentId", form.getValues("departmentId"));
      }
    }
  }, [departments, isAddDialogOpen, isEditDialogOpen]);
