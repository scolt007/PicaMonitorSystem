...
const ProjectSite: React.FC = () => {
  ...
  // Form for creating/editing project sites
  ...
  // Update project site mutation
  const updateProjectSite = useMutation({
    mutationFn: async (data: FormValues) => {
      if (!selectedSite) throw new Error("No project site selected");
      return apiRequest("PUT", `/api/project-sites/${selectedSite.id}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/project-sites"] });
      toast({
  ...
  // Delete project site mutation
  const deleteProjectSite = useMutation({
    mutationFn: async () => {
      if (!selectedSite) throw new Error("No project site selected");
      return apiRequest("DELETE", `/api/project-sites/${selectedSite.id}`, undefined);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/project-sites"] });
      toast({
        title: "Success",
        description: "Project site deleted successfully",
      });
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-gray-800">Project Site</h1>
      </div>
            ...
            <table className="min-w-full bg-white">
              <thead>
                <tr>
                  ...
                  <th className="py-2 px-4 bg-primary text-white text-left text-sm font-medium">Site Name</th>
                  <th className="py-2 px-4 bg-primary text-white text-left text-sm font-medium">Location</th>
                  <th className="py-2 px-4 bg-primary text-white text-left text-sm font-medium">Site Manager</th>
            ...
            <form onSubmit={form.handleSubmit(onSubmitCreate)} className="space-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} placeholder="Enter site code (e.g., KMP-NPR)" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <DialogFooter>
                ...
                <Button type="submit" disabled={createProjectSite.isPending}>
                  {createProjectSite.isPending ? "Adding..." : "Add Project Site"}
                </Button>
              </DialogFooter>
            </form>
          </Form>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          ...
          <p>
            Are you sure you want to delete <strong>{selectedSite?.name}</strong> project site? This action cannot be undone.
          </p>
          <DialogFooter>
            <Button
              type="button"
